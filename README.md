...
- `CDERUN_GROUP_ADD`: Supplementary group IDs or names to add to the execution user.
- `CDERUN_READ_ONLY`: Mount the container's root filesystem as read-only.
- `CDERUN_INIT`: Run an init process inside the container to forward signals and reap zombie processes.
- `CDERUN_PID`: Configure PID namespace (`host` or private).
...
- `CDERUN_GPUS`: GPU devices to request (`all`, `count=2`, `device=0,1`).
