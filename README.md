...
- `--pid`: Configure the PID namespace for the container. Accepts `"host"` or `""` (private). Maps to `PidMode` in Docker host configuration and `specs.PIDNamespace` in containerd OCI spec when configured as `"host"`. (Default: `""`)
