...
- `--shm-size`: Configure the size of `/dev/shm` for containers. Maps to `ShmSize` in the Docker host configuration or Podman host configuration, and dynamically updates the `/dev/shm` tmpfs mount in the containerd OCI specification via helper function `UpdateShmSize`, giving the configured `shmSize` complete precedence over any existing `/dev/shm` mounts. (Default: none)
