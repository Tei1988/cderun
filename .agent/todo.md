...
Migrate from `fmt.Errorf` gradually.
Done: `ImageNotFoundError`, `InvalidConfigError`, `RegistryMismatchError` in `internal/config`.
Todo: `RuntimeInitError` for `internal/runtime`.
