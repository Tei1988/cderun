...
### P-6: Typed Error Handling (In Progress)

Typed errors introduced for `internal/config`. Need to migrate other packages incrementally.
- [x] internal/config (ImageNotFoundError, InvalidConfigError, RegistryMismatchError)
- [ ] internal/runtime
- [ ] internal/command

Usage: `return nil, &ImageNotFoundError{Tool: subcommand}`
Tests: `require.ErrorAs(t, err, &target)`
