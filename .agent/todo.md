...
| T85 | コンフォーマンス（L3: Conformance Suite）の導入による各ランタイム間の挙動不変性保証 | テスト | 中 | 大 | - | - |
| T86 | ゴールデンテスト（L2: Golden Tests）の複合シナリオの追加 | テスト | 低 | 小 | - | - |
...
## T85: コンフォーマンス（L3: Conformance Suite）の導入による各ランタイム間の挙動不変性保証
...
`ContainerRuntime` インターフェースを実装する全アダプタ（Mock, Docker, Podman, containerd）が同一の `ContainerConfig` 入力に対して、同一の挙動をすること、または未対応の機能について確実にエラーを返す仕様（契約）を満たしていることを一元的にテストする。

### 完了条件

- [ ] 共通の適合テストスイート（例: `conformance_suite_test.go` の `RunConformanceTests`）を実装する。
- [ ] テストケースには、「Entrypoint 未指定時にイメージの値が正しく前置されること」「未対応のポートマウントやボリュームに対して正しく `not supported` エラーが返ること」など、共通の契約検証項目を含める。
- [ ] `MockRuntime` で毎PR時にこのスイートが実行され、CIの実機ランタイムテスト（`-tags=runtime`）でも同じテストケースを流せるように設計する。
