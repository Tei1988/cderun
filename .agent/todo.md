...
3. **Docker ジョブの追加**: `pull_request` コードがホストの `/var/run/docker.sock` に直接アクセスしないよう構成する。使い捨て（disposable）やルートレス（rootless）Docker デーモンの使用、または信頼されたリファレンス（trusted refs）のみへのジョブ制限を行い、必要な Docker ビルドおよびテスト機能を維持しつつ安全性を確保する。
...
- `Server.Close()` のシャットダウン契約として実現可能な有界待機（bounded-wait）ポリシーを定義する: ディスパッチャー操作が `s.wg.Wait()` に入る前にコンテキストキャンセル（`ctx.Done()`）を尊重するようにし、設定されたタイムアウト/デッドライン経過後に迅速に復帰（return）する。期限内に終了しないハンドラが存在する場合は、強制終了可能であると誤認させず、ログ出力およびドキュメントにて明記する。

### 完了条件

- 長時間ブロックする RPC（`WaitContainer`）の実行中にクライアント接続を切断すると、サーバ側のハンドラが速やかに終了するテストがある
- `Server.Close()` が有界待機ポリシーに従い、設定されたデッドライン内に迅速に復帰することを確認するテストがある（未終了ハンドラが存在する場合もハングせずログ出力して復帰すること）
...
- ハンドシェイク成立後の接続にアイドルタイムアウトを設ける。ただし `WaitContainer` および `AttachContainer` のように長時間無通信になり得る RPC / ストリーミング接続を接続状態（connection-state）または RPC 分類機構を用いてアイドルタイムアウト適用対象から除外（免除）し、通常のアイドル接続に対するタイムアウト挙動は維持する。
...
- `WaitContainer` および対話実行（`AttachContainer`）中の接続がアイドルタイムアウト適用から除外され切断されないことを確認するリグレッションテスト（長時間バイト未送信インターバルを含む）がある
- `make build` / `make test` / `make lint-go` がパスする
...
- 依存: なし
- スコープ: `AGENTS.md` と `docs/testing/organization.md` への禁止語（`improvement`, `expansion`, `refinement`, `comprehensive`, `additional`, `extra`, `more`, `deep`, `jules` 等のエージェント名）および命名例の明文化。禁止語を含むファイル名を検知するリントコマンド（`make lint-test-names` 等）の作成と CI ワークフローへの必須チェックとしての組み込み。
- 完了条件: リントコマンドおよび CI ジョブが追加・必須化され、規約が明文化されていること。
