...
## T03: アンカー検証と `~` 展開の不整合修正

- 種別: バグ修正
- 対象:
  - `internal/config/path.go:342`（`magicWordPreRegex`）
  - `internal/config/path.go:505`（`validateAnchorBoundaries`）
  - `internal/config/expression.go:226`（`~` 展開ロジック）

### 問題

`magicWordPreRegex`（`(^~|[/\\]~)`）はパス途中の `~`（例: `/foo/~bar`）もアンカーとして検出するが、`expression.go` の展開ロジックは文字列先頭の `~` のみ展開する。この不整合により、実際には展開されないパスがパストラバーサル検証エラーになり得る。

### 方針（2 択）

1. `magicWordPreRegex` から `[/\\]~` を外し `^~` のみにする（展開仕様に合わせる）— 単純
2. `validateAnchorBoundaries` 側で「実際に展開が起きたか」（original と resolved の差分）を確認してから境界チェックを行う — 「パス中間の `~` を将来展開するかもしれない」前提ならこちら

特段の事情がなければ (1) を推奨。

### 完了条件

- `/foo/~bar` のような中間 `~` を含むパスが誤って検証エラーにならない
- 先頭 `~` の展開＋境界検証は従来どおり機能する（回帰テスト）
