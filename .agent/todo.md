...
### Add `--read-only` documentation
- **Description**: Add reference and description for the new `--read-only` and `--cderun-read-only` flags.
- **Files to update**:
  - `docs/features/argument-priority-logic.md` (Add `--read-only` / `CDERUN_READ_ONLY` to the priority list)
  - `docs/features/command-line-options.md` (Add option explanation for `--read-only`)
  - `docs/features/configuration-file-support.md` (Add YAML schema description for `readOnly`)
