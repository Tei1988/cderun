...
| T81 | 引数ホイストにおけるダブルダッシュ（`--`）の制限廃止 | 改善 | 高 | 中 | あり | DONE |
| T82 | デフォルトログレベルのerror化 | 改善 | 高 | 小 | - | DONE |
| T83 | コンテナ起動成否によるログ重要度再分類およびアタッチ・終了関連のロジック・テスト修正 | 改善 | 高 | 中 | - | WIP |
...
## T83: コンテナ起動成否によるログ重要度再分類およびアタッチ・終了関連のロジック・テスト修正
...
起動成否をベースにしたログレベルの再分類：
- コンテナが起動できない/起動を継続できないエラー（起動前、StartContainer の失敗など）: `error` として致命的エラーを返し、メインプロセスで stderr 出力する。
- コンテナは起動できる/起動継続できる警告（AttachContainer 失敗、SIGKILL による強制終了失敗など）: `warn` レベルとしてロガーに記録し、致命的エラーとして呼び出し元には返さない。
- ただし、アタッチエラーやSIGKILL強制終了のシグナル送信時に、タイムアウトなどで正常なコンテナ終了が確認できない場合は、呼び出し元にエラー（125）を返し、成功扱いでの早期終了を防ぐ。

### 完了条件

- コンテナ起動完了後の警告（アタッチエラー等）が致命的エラーではなく、Warn ログへと再分類されている。
- ハングタイムアウト発生時やSIGKILL後の未終了時は、成功とみなさずExitCodeError（125）を返す。
- 関連するテストがすべて正常にパスしている。
