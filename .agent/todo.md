...
### P-6: Typed Error Handling (Partial)

`internal/config` has been refactored to use typed errors (`ImageNotFoundError`, `InvalidConfigError`, `RegistryMismatchError`).

Next:
- Migrate `RuntimeInitError` for runtime initialization failures in `internal/command/root.go`.
- Expand typed errors to other packages (`internal/runtime`, etc.) where beneficial.
