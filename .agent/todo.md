...
| T81 | 引数ホイストにおけるダブルダッシュ（`--`）の制限廃止、デフォルトログレベルのerror化、及びコンテナ起動成否によるログ重要度再分類 | 改善 | 高 | 中 | あり | WIP |
...
## T81: 引数ホイストにおけるダブルダッシュ（`--`）の制限廃止、デフォルトログレベルのerror化、及びコンテナ起動成否によるログ重要度再分類
...
2. デフォルトログレベルを `"warn"` から `"error"` に引き上げ、標準実行時のセキュリティ警告（Warnレベル）や実行開始ログ（Infoレベル）などの無用な出力を抑制する。
3. 起動成否をベースにしたログレベルの再分類：
   - コンテナが起動できない/起動を継続できないエラー（起動前、StartContainer の失敗など）: `error` として致命的エラーを返し、メインプロセスで stderr 出力する。
   - コンテナは起動できる/起動継続できる警告（AttachContainer 失敗、SIGKILL による強制終了失敗など）: `warn` レベルとしてロガーに記録し、致命的エラーとして呼び出し元には返さない（デフォルトログレベル `error` のため、通常の標準実行時は stderr にエラー出力されない。明示的に指定された場合のみ出力される）。
   - 警告でもなく、起動可否の致命的エラーでもない、純粋なステータス情報ログ（`Running: ...` や `Pulling image ...` などの実行中・ダウンロード中ログ）: 警告のない実行中の通常出力と不要なノイズを防ぐため、`info` レベルから `debug` レベルへと引き下げる（これらはログレベル debug 以上を指定した時のみ表示される）。

### 完了条件

- `preprocessArgs` から `doubleDashFound` によるホイスト無効化のロジックが完全に除去されている。
- デフォルトログレベルが `"error"` に更新されている。
- コンテナ起動完了後の警告（アタッチエラー等）が致命的エラーではなく、Warn ログへと再分類されている。
- 通常ステータスログ（`Running: ...` や `Pulling image ...`）が `info` から `debug` レベルに変更されている。
- 関連するすべてのテスト及びドキュメントが更新・パスしている。
