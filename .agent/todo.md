...
| T99 | テストファイル命名規約の強制 | クリーンアップ | 高 | 大 | - | - |
...
3. **Docker ジョブの追加**: runner の `/var/run/docker.sock` をそのまま使う。runner ユーザーは docker グループ所属済みのため ACL 設定は不要のはず
...
- `--oci-runtime` / `CDERUN_OCI_RUNTIME` / `.cderun.yaml` で OCI ランタイムを指定できる
...
- `Server.Close()` は有限時間で必ず返ること。ハンドラが期限内に終わらない場合の扱い（強制的に諦めてログを残す等）を決めて実装する

### 完了条件

- 長時間ブロックする RPC（`WaitContainer`）の実行中にクライアント接続を切断すると、サーバ側のハンドラが速やかに終了するテストがある
- 上記の状況で `Server.Close()` が有限時間で返ることを確認するテストがある（現状はハングするため、修正前は失敗するテストであること）
...
- ハンドシェイク成立後の接続にアイドルタイムアウトを設ける。ただし `AttachContainer` のように長時間無通信になり得るストリーミング接続を誤って切らないこと（対話実行中の接続はタイムアウト対象から外すか、十分長い値にする）
...
- 対話実行（`AttachContainer`）中の接続がアイドルタイムアウトで切断されないことを確認するテストがある
- `make build` / `make test` / `make lint-go` がパスする
...
1. **禁止語を明文化する**: ファイル名に `improvement` / `expansion` / `refinement` / `comprehensive` / `additional` / `extra` / `more` / `deep` / エージェント名（`jules` 等）を含めない。`AGENTS.md` の該当節と `docs/testing/organization.md` 3.3 の両方に書く
2. **既存83ファイルをリネームする**: 中身を読み、何を検証しているかに基づいて `feature_*` / `bugfix_*` / `<対象>_robustness_*` 等へ改名する。1つのファイルが複数テーマを含む場合は分割する
3. **リネーム中に重複テストを削除する**: 同一の入力・同一の assertion を別ファイルで繰り返しているものは1つに統合する。削除したテストは PR 説明に列挙する
4. 機械的チェックを CI に追加できるか検討する（禁止語を含むファイル名があれば失敗させる lint）

### 完了条件

- 禁止語を含むテストファイル名が 0 件
- `AGENTS.md` と `docs/testing/organization.md` に禁止語と命名例が明記されている
- 重複削除の結果、テスト行数が削減されている（削減率は問わないが、削除したテストの一覧が PR 説明にある）
- `make test` の結果が変更前と同じ（カバレッジが有意に下がっていないこと）
- `make lint-go` / `make lint-md` がパスする

### 進め方の注意

- 一度に83ファイルを触ると巨大 PR になりコンフリクトも起きる。パッケージ単位（`internal/config` → `internal/command` → `internal/runtime`）で PR を分けること
- リネームのみの PR と、重複削除を含む PR を分けると差分が読める。**先にリネームだけを済ませ、重複削除は次の PR にする**ことを推奨
