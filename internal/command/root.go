...
func maskSensitiveEnv(envs []string) []string {
	res := make([]string, len(envs))
	sensitiveKeywords := []string{"PASSWORD", "SECRET", "TOKEN", "KEY", "AUTH", "SIG"}
...
func (o *rootOptions) handleDryRun(cmd *cobra.Command, containerConfig *container.ContainerConfig, resolved *config.ResolvedConfig) error {
	...
	maskedConfig := *containerConfig
	maskedConfig.Env = maskSensitiveEnv(containerConfig.Env)

	return o.writeFormatted(cmd.OutOrStdout(), resolved.DryRunFormat, &maskedConfig, func(w io.Writer) {
		_, _ = fmt.Fprintf(w, "Image: %s\n", maskedConfig.Image)
		_, _ = fmt.Fprintf(w, "Command: %s\n", strings.Join(maskedConfig.Command, " "))
		_, _ = fmt.Fprintf(w, "TTY: %v\n", maskedConfig.TTY)
		_, _ = fmt.Fprintf(w, "Interactive: %v\n", maskedConfig.Interactive)
		_, _ = fmt.Fprintf(w, "Network: %s\n", maskedConfig.Network)
		_, _ = fmt.Fprintf(w, "Remove: %v\n", maskedConfig.Remove)
		var mounts []string
		for _, m := range maskedConfig.Mounts {
		...
		_, _ = fmt.Fprintf(w, "Env: %s\n", strings.Join(maskedConfig.Env, ", "))
		_, _ = fmt.Fprintf(w, "Workdir: %s\n", maskedConfig.Workdir)
		_, _ = fmt.Fprintf(w, "User: %s\n", maskedConfig.User)
		_, _ = fmt.Fprintf(w, "Ports: %s\n", strings.Join(maskedConfig.Ports, ", "))
		_, _ = fmt.Fprintf(w, "PublishAll: %v\n", maskedConfig.PublishAll)
		_, _ = fmt.Fprintf(w, "Expose: %s\n", strings.Join(maskedConfig.Expose, ", "))
		_, _ = fmt.Fprintf(w, "Hostname: %s\n", maskedConfig.Hostname)
		_, _ = fmt.Fprintf(w, "DNS: %s\n", strings.Join(maskedConfig.DNS, ", "))
		_, _ = fmt.Fprintf(w, "AddHosts: %s\n", strings.Join(maskedConfig.AddHosts, ", "))
		_, _ = fmt.Fprintf(w, "Privileged: %v\n", maskedConfig.Privileged)
		_, _ = fmt.Fprintf(w, "CapAdd: %s\n", strings.Join(maskedConfig.CapAdd, ", "))
		_, _ = fmt.Fprintf(w, "CapDrop: %s\n", strings.Join(maskedConfig.CapDrop, ", "))
		_, _ = fmt.Fprintf(w, "Entrypoint: %s\n", strings.Join(maskedConfig.Entrypoint, ", "))
		_, _ = fmt.Fprintf(w, "Pull: %s\n", maskedConfig.Pull)
		_, _ = fmt.Fprintf(w, "Memory: %s\n", units.BytesSize(float64(maskedConfig.Memory)))
		_, _ = fmt.Fprintf(w, "CPUs: %g\n", maskedConfig.CPUs)
		var devices []string
		for _, d := range maskedConfig.Devices {
