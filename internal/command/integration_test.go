...
`
	...
	_, err = executeCommandWithOptions(context.Background(), []string{"node", "--version"}, setup)
...
`
	...
	_, err = executeCommandWithOptions(context.Background(), []string{"cderun", "node", "app.js"}, setup)
	require.NoError(t, err)
...
`
	...
	_, err = executeCommandWithOptions(context.Background(), []string{"cderun", "--mount-socket", "--mount-tools", "unknown", "--image", "alpine", "sh"}, setup)
	require.Error(t, err)
...
`
	...
	_, err = executeCommandWithOptions(context.Background(), []string{"cderun", "--image", "alpine", "--mount-tools", "node", "sh"}, setup)
	require.NoError(t, err)
	require.NotNil(t, mockRuntime.CreatedConfig)
...
`
	...
	_, err = executeCommandWithOptions(context.Background(), []string{"cderun", "--mount-tools", "node", "--mount-socket", "--socket-path", "/socket", "sh"}, setup)
	require.NoError(t, err)
	...
	_, err = executeCommandWithOptions(context.Background(), []string{"cderun", "--mount-all-tools", "--mount-socket", "--socket-path", "/socket", "sh"}, setup)
	require.NoError(t, err)
...
func TestIntegration_Command_Root_MountAllTools_EmptyConfig(t *testing.T) {
	...
	output, err := executeCommandWithOptions(context.Background(), []string{"cderun", "--mount-all-tools", "--mount-socket", "--socket-path", "/socket", "--image", "alpine", "sh"}, setup)
	require.NoError(t, err)
...
`
	...
	t.Run("cderun-tty overrides tty even if placed after subcommand", func(t *testing.T) {
		_, err := executeCommandWithOptions(context.Background(), []string{"cderun", "--tty=true", "node", "--cderun-tty=false", "--version"}, setup)
		require.NoError(t, err)
	...
	t.Run("cderun-tty works in polyglot mode", func(t *testing.T) {
		mockRuntime.CreatedConfig = nil
		_, err := executeCommandWithOptions(context.Background(), []string{"node", "--cderun-tty=true", "--version"}, setup)
		require.NoError(t, err)
	...
	t.Run("cderun internal overrides before subcommand result in error", func(t *testing.T) {
		_, err := executeCommandWithOptions(context.Background(), []string{"cderun", "--cderun-image=alpine:latest", "sh"}, setup)
		require.Error(t, err)
	...
	t.Run("cderun internal overrides after subcommand work correctly", func(t *testing.T) {
		mockRuntime.CreatedConfig = nil
		_, err := executeCommandWithOptions(context.Background(), []string{"cderun", "--image=alpine:stable", "sh", "--cderun-image=alpine:latest"}, setup)
		require.NoError(t, err)
		require.NotNil(t, mockRuntime.CreatedConfig)
	...
	t.Run("cderun internal overrides for network, remove, workdir and mount", func(t *testing.T) {
		mockRuntime.CreatedConfig = nil
		_, err := executeCommandWithOptions(context.Background(), []string{"cderun", "--image=alpine", "--network=bridge", "--remove=false", "--workdir=/initial", "--mount=type=bind,source=/h1,target=/c1", "sh", "--cderun-network=host", "--cderun-remove=true", "--cderun-workdir=/override", "--cderun-mount=type=bind,source=/h2,target=/c2"}, setup)
		require.NoError(t, err)
		require.NotNil(t, mockRuntime.CreatedConfig)
		assert.Equal(t, "host", mockRuntime.CreatedConfig.Network)
	...
	t.Run("cderun internal overrides for runtime, socket and mounting", func(t *testing.T) {
		...
		_, err = executeCommandWithOptions(context.Background(), []string{"cderun", "--image=alpine", "sh", "--cderun-runtime=docker", "--cderun-socket-path=/var/run/custom.sock", "--cderun-mount-socket=true", "--cderun-mount-cderun=true", "--cderun-mount-tools=node"}, setup)
		require.NoError(t, err)
		require.NotNil(t, mockRuntime.CreatedConfig)
	...
	t.Run("cderun internal override can turn off remove", func(t *testing.T) {
		mockRuntime.CreatedConfig = nil
		_, err := executeCommandWithOptions(context.Background(), []string{"cderun", "--image=alpine", "--remove=true", "sh", "--cderun-remove=false"}, setup)
		require.NoError(t, err)
		require.NotNil(t, mockRuntime.CreatedConfig)
	...
	t.Run("cderun internal overrides for dry-run", func(t *testing.T) {
		mockRuntime.CreatedConfig = nil
		output, err := executeCommandWithOptions(context.Background(), []string{"cderun", "--image=alpine", "sh", "echo", "hello", "--cderun-dry-run", "--cderun-dry-run-format=simple"}, setup)
		require.NoError(t, err)
...
func TestIntegration_Command_Stdin_Mocked(t *testing.T) {
	...
	stdout, stderr, exitCode, err := runCderunCoreWithOptions(pr, setup, "--image", "alpine", "-i", "cat")
