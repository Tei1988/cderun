...
func TestUnit_Root_Snapshot_Immutability(t *testing.T) {
...
func TestUnit_Root_Snapshot_WithNilHostContext(t *testing.T) {
	mfs := &config.MockFileSystem{}
	globalCfg := &config.CDERunConfig{
		Runtime: "docker",
	}
...
func TestUnit_Root_Snapshot_Permissions(t *testing.T) {
	mfs := &config.MockFileSystem{}
	globalCfg := &config.CDERunConfig{}
	toolsCfg := config.ToolsConfig{}
	currentMounts := []container.Mount{}
...
func TestUnit_Root_Snapshot_DiscoverOverlay(t *testing.T) {
	mfs := &config.MockFileSystem{}
