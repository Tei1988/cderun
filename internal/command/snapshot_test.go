...
func TestUnit_Command_Snapshot_Immutability(t *testing.T) {
	...
	snapshotDir, err := createSnapshot(mfs, &mockMountInfoReader{}, globalCfg, toolsCfg, currentMounts)
...
func TestUnit_Command_Snapshot_DiscoverOverlay(t *testing.T) {
	...
	t.Run("successfully discover upperdir", func(t *testing.T) {
		...
		miReader := &mockMountInfoReader{Content: []byte(mountinfo)}

		upperdir, err := discoverOverlayUpperDir(mfs, miReader)
		require.NoError(t, err)
