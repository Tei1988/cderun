...
func TestUnit_Snapshot_Immutability(t *testing.T) {
	...
	snapshotDir, err := createSnapshotWithReader(logging.NewLogger(), mfs, globalCfg, toolsCfg, currentMounts, &mockMountInfoReader{})
...
func TestUnit_Snapshot_DiscoverOverlay(t *testing.T) {
	...
	t.Run("successfully discover upperdir", func(t *testing.T) {
		...
		upperdir, err := discoverOverlayUpperDirWithReader(mfs, reader)
		require.NoError(t, err)
