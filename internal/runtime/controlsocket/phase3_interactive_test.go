...
func TestUnit_ControlSocket_Phase3_AttachContainer_NonTTY_StdCopy(t *testing.T) {
	...
	disp := &mockDispatcher{
		attachFunc: func(ctx context.Context, containerID string, tty bool, stdin io.Reader, stdout, stderr io.Writer, ready chan<- struct{}) error {
			...
			// In non-TTY mode, server multiplexes stdout and stderr using stdcopy
			outWriter := stdcopy.NewStdWriter(stdout, stdcopy.Stdout)
			errWriter := stdcopy.NewStdWriter(stdout, stdcopy.Stderr)

			_, _ = outWriter.Write([]byte("stdout data\n"))
			_, _ = errWriter.Write([]byte("stderr data\n"))
			return nil
		},
	}
