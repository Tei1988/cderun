...
type ContainerdRuntime struct {
	...
	mu         sync.Mutex
	containers map[string]*containerIO
}
...
func NewContainerdRuntime(socket string) (*ContainerdRuntime, error) {
	...
	return &ContainerdRuntime{
		...
		socket:     socket,
		name:       "containerd",
		containers: make(map[string]*containerIO),
	}, nil
}
...
func (r *ContainerdRuntime) getIO(id string) *containerIO {
	r.mu.Lock()
	defer r.mu.Unlock()
	if io, ok := r.containers[id]; ok {
		return io
	}
	io := &containerIO{
...
func (r *ContainerdRuntime) deleteIO(id string) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ContainerdRuntime) CreateContainer(ctx context.Context, config *container.ContainerConfig) (string, error) {
	// Preliminary validations for unsupported features in this initial implementation
...
func (r *ContainerdRuntime) StartContainer(ctx context.Context, containerID string) error {
	...
	cioIO := r.getIO(containerID)
	creator := cioIO.creator
	if creator == nil {
...
func (r *ContainerdRuntime) WaitContainer(ctx context.Context, containerID string) (int, error) {
	...
	cioIO := r.getIO(containerID)
	// Signal that the task has finished to unblock AttachContainer
	cioIO.wait <- status.Error()
...
func (r *ContainerdRuntime) AttachContainer(ctx context.Context, containerID string, tty bool, stdin io.Reader, stdout, stderr io.Writer, ready chan<- struct{}) error {
	...
	cioIO := r.getIO(containerID)
	cioIO.creator = cio.NewCreator(ioOpts...)
