...
func addEnvSmall(keys *[16]string, vals *[16]string, size *int, env []string) {
	for _, e := range env {
		key, _, _ := strings.Cut(e, "=")
		foundIdx := -1
		for j := 0; j < *size; j++ {
...
func resolveEnvValues(env []string, sensitivePatterns []string, strict bool, r *ExpressionResolver, fs FileSystem) ([]string, error) {
	...
	for i, e := range env {
		...
		final := ""
		if resolvedE == e && hasValue {
			final = e
		} else if len(e) == len(key)+1+len(val) && strings.HasPrefix(e, key) && e[len(key)] == '=' && e[len(key)+1:] == val {
			final = e
		} else {
			final = key + "=" + val
		}
