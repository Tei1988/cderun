...
func TestUnit_ResolveEnvValues_ControlCharacterBoundaryValidation(t *testing.T) {
	...
	t.Run("rejected ASCII control characters with exact position and error message", func(t *testing.T) {
		// Control char \x01 at position 3 ('A'=0, 'B'=1, 'C'=2, \x01=3)
	...
	t.Run("rejected ASCII DEL character (0x7f) with exact position and error message", func(t *testing.T) {
		// DEL \x7f at position 2 ('X'=0, 'Y'=1, \x7f=2)
	...
	t.Run("rejected C1 Unicode control characters with exact rune position", func(t *testing.T) {
		// U+0080 (Control C1) at position 4 ('P'=0, 'A'=1, 'S'=2, 'S'=3, U+0080=4)
