...
func TestUnit_Coverage_Resolver_ResolveWithFS_RegistryMismatch(t *testing.T) {
	...
	_, err = ResolveWithFS("sh", &cli, nil, nil, &MockFileSystem{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "registry mismatch: info not found for option \"hc\"")
	...
	_, err = ResolveWithFS("sh", &cli, nil, nil, &MockFileSystem{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "registry mismatch: info not found for option \"bad-bool\"")
}
...
func TestUnit_Coverage_Resolver_ResolveWithFS_RegistryMismatchErrors(t *testing.T) {
	...
	_, err := ResolveWithFS("sh", &cli, nil, nil, mfs)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "registry mismatch: early boolean option not found for option \"diagnosis\"")
}
