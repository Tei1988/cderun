...
func fetchFieldAndParams(key string, cliVal reflect.Value) (optionFields, bool, reflect.Value, bool, reflect.Value, error) {
	...
	if !ok {
		return optionFields{}, false, reflect.Value{}, false, reflect.Value{}, &RegistryMismatchError{Option: key, Reason: "info not found"}
	}

	if info.p1ValIdx == nil || info.p2ValIdx == nil {
		return optionFields{}, false, reflect.Value{}, false, reflect.Value{}, &RegistryMismatchError{Option: key, Reason: "CLI reflection fields missing in CLIOptions"}
	}
...
func ResolveWithFS(subcommand string, cli *CLIOptions, tools ToolsConfig, global *CDERunConfig, fs FileSystem) (*ResolvedConfig, error) {
	...
	for _, name := range []string{"diagnosis", "strict-env"} {
		...
		if !ok {
			return nil, &RegistryMismatchError{Option: name, Reason: "early boolean option not found"}
		}
		def := OptionDef[*bool]{
			EnvKey:       opt.EnvKey,
