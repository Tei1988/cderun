...
3. **Phase 3 — Interactive I/O (Attach, Signal, Resize) (Completed)**: RPC dispatch handlers for `AttachContainer` (hijacked stdin/stdout/stderr stream demuxing or raw TTY passthrough), `SignalContainer` (OS signal forwarding), and `ResizeContainerTTY` (PTY window resize) over dedicated Control Socket connections to the active runtime adapter. (Implemented in T89)
