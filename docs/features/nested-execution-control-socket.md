...
3. **Phase 3 — Interactive I/O (Attach, Signal, Resize)**: stdin/stdout/stderr multiplexing, signal forwarding, and TTY resize over the Control Socket. Currently, `ControlSocketRuntimeAdapter` dispatches non-interactive Phase 2 lifecycle methods over the Control Socket while interactive `AttachContainer`, `SignalContainer`, and `ResizeContainerTTY` methods delegate directly to the underlying runtime pending Phase 3 Control Socket RPC completion.
