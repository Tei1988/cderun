...
2. **Phase 2 — Docker Dispatch (Non-Interactive)**: `CreateContainer` / `StartContainer` / `WaitContainer` / `RemoveContainer` routed over the Control Socket to the Docker adapter (`ControlSocketRuntimeAdapter`). First phase where the mechanism is actually functional, for non-TTY/non-interactive execution (Implemented in T88).
