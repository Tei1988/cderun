...
3. **Phase 3 — Interactive I/O (Attach, Signal, Resize) (Completed)**: stdin/stdout/stderr multiplexing, signal forwarding, and TTY resize over the Control Socket. `ControlSocketRuntimeAdapter` dispatches interactive `AttachContainer`, `SignalContainer`, and `ResizeContainerTTY` methods via Control Socket RPC. (Implemented in T89)
