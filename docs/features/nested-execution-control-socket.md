...
1. **Phase 1 — Protocol and Socket Plumbing (Completed)**: Control Socket creation, directory snapshot bind mounting (`/run/cderun/cderun.sock`), cleanup deferral, binary length-prefixed JSON framing (`MaxFrameSize = 16MB`), handshake version negotiation (`Version 1`), ping/pong heartbeats, mutex-synchronized connection tracking, exponential backoff retries on temporary accept errors, connection idle timeouts (default 60s), request context cancellation on peer disconnect, and long-running operation idle deadline exemptions (`WaitContainer` and `AttachContainer`). (Implemented in T87, T96, T97)
...
3. **Phase 3 — Interactive I/O (Attach, Signal, Resize) (Completed)**: RPC dispatch handlers for interactive stream demuxing (`AttachContainer`), process signal forwarding (`SignalContainer`), and TTY terminal window resizing (`ResizeContainerTTY`) over the Control Socket. `ControlSocketRuntimeAdapter` dispatches interactive calls directly to `Client` over dedicated streaming Control Socket connections. (Implemented in T89)
