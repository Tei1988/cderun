...
1. **Phase 1 — Protocol and Socket Plumbing (Completed)**: Control Socket creation, directory snapshot bind mounting (`/run/cderun/cderun.sock`), cleanup deferral, binary length-prefixed JSON framing (`MaxFrameSize = 16MB`), handshake version negotiation (`Version 1`), ping/pong heartbeats, and mutex-synchronized connection tracking. (Implemented in T87)
2. **Phase 2 — Docker Dispatch (Non-Interactive) (Completed)**: RPC dispatch handlers for `CreateContainer`, `StartContainer`, `WaitContainer`, and `RemoveContainer` over Control Socket to the Docker adapter (`ControlSocketRuntimeAdapter`), backed by auto-detection of `HostContext.ControlSocket` and `/run/cderun/cderun.sock`. (Implemented in T88)
3. **Phase 3 — Interactive I/O (Attach, Signal, Resize) (Completed)**: RPC handlers and message types (`MsgAttachContainer`, `MsgSignalContainer`, `MsgResizeContainerTTY`) for streaming connection hijacking, signal forwarding, and TTY window size synchronization over the Control Socket. `Server` handles connection hijacking for `MsgAttachContainer` with thread-safe `stdcopy` stream demuxing (non-TTY) or raw stream passthrough (TTY), while `Client.dialAndHandshake` opens dedicated streaming connections to keep concurrent RPCs (`SignalContainer`, `ResizeContainerTTY`) responsive over the Control Socket. Delegated via `ControlSocketRuntimeAdapter`. (Implemented in T89)
