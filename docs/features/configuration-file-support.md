...
設定ファイル内に記述されたパスや変数は、実行前に解決されます。
...
#### マジックワード
設定ファイル内では `{{WORD}}` という形式で、cderunが提供する特別な「マジックワード」を使用できます。これにより、環境に依存する値を動的に埋め込むことができます。
...
**解決の優先順位:**
1. マジックワードの展開が先に行われます。
2. その後、結果のパスが相対パスであれば、ファイル場所を基準に解決されます。
3. マジックワード展開後、および相対パス解決後のパス文字列は、標準的なパス正規化ルールに従って処理されます (例: `{{PWD}}/../src` は `/path/to/src` に正規化されます)。
