...
1.  **cderun Expressions の展開**:
    `{{...}}` 式が評価され、対応する値に置換されます。
2.  **相対パスの解決**:
    上記の結果、値が相対パス形式 (`./` or `../`) になった場合、その設定ファイルが置かれている場所を基準に絶対パスに変換されます。
3.  **パスの正規化**:
    最終的なパス文字列は、標準的なパス解決ルールに従って正規化されます (例: `/path/to/work/../src` は `/path/to/src` になります)。
