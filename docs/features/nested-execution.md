...
### 3. 指定される GID の規則 (数値型限定)

`groupAdd` または `--cderun-group-add` に渡されるソケットアクセス用補助グループは、**数値 GID**（例: `"102"`）として指定される必要があります。直接 containerd 実行等の際にコンテナ内での名前解決（例: `"docker"`）が行われない変換仕様になっているため、数値 GID 以外の文字列を渡した場合はエラーとなります。
...
- **ソケットマウント時の警告ロギング**: コンテナソケットのマウント（`MountSocket`）を有効にすると、`Warn` レベルのセキュリティ警告ログが出力されます。
- **数値 GID 警告**: ソケットマウントが有効であり、かつ数値型の補助グループ（`GroupAdd`）が指定されている場合、ホストシステム全体への広範なデーモン操作権限が与えられるセキュリティリスクを強調する特定の警告がログに記録されます。
- **特権アクセスの最小化**: rootless Podman であっても、ソケットを共有することで対象ユーザーのコンパイル済みのリソース制御が実質的に可能になるため、必要最低限の権限のみを付与するよう設計してください。
