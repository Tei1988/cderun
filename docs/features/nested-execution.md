...
### 4. 厳格な検証警告（Least Privilege とソケットマウント警告）

`cderun` は、不必要な特権付与や誤設定によるセキュリティリスクを最小限に抑えるため、自動的な検証・警告メカニズムを備えています。

- **ソケットマウントに対する警告**:
  `--mount-socket`（または `--mount-cderun`）を有効にすると、`cderun` は起動時に自動的に `Warn` レベルの警告メッセージをログに出力し、コンテナ内からホストデーモンを操作可能にすることのセキュリティリスクを管理者に再認させます。
- **数値GID（VM GID）マッピング警告**:
  ソケットマウントを有効化した状態で、さらに数値形式の補助グループ（`groupAdd`）が指定されている場合（例: `"102"` などの GID）、VM 上のソケット所有権への特権的なアクセスパスがコンテナ内プロセスに開かれることになります。この組み合わせを検出すると、`cderun` は **「数値 GID アクセスに伴うコンテナエスケープやホスト特権奪取のリスク」** について明確な個別の警告をログに表示します。補助グループに数値 GID が含まれているかどうかの判定は、一元化された判定ヘルパー（`ContainsNumericGID`）により高精度に検証されます。
