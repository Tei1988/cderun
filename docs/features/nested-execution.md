...
1. **バイナリマウント (`--mount-cderun`, `--mount-cderun-path`)**: 基底ホストの `cderun` バイナリがコンテナ内にマウントされます（通常は `/usr/local/bin/cderun`）。`--mount-cderun-path` を使用することで、マウントするホスト側のバイナリを明示的に指定できます。
...
- **作成**: `--mount-cderun`, `--mount-tools`, `--mount-all-tools` のいずれかを伴う実行、または既にネストレベルが 1 以上の環境での実行において、コンテナ起動プロセスの途中で生成されます。
- **削除**: コンテナが終了（または `cderun` プロセスが終了）した際に、実行ホスト上の `/tmp/cderun-snapshots-<uuid>/` を再帰的に削除します。
