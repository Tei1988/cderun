...
1. **バイナリマウント (`--mount-cderun`, `--mount-cderun-path`, `--mount-tools`, `--mount-all-tools`)**: 基底ホストの `cderun` バイナリがコンテナ内にマウントされます（通常は `/usr/local/bin/cderun`）。`--mount-cderun-path` を使用することで、マウントするホスト側のバイナリを明示的に指定できます。
   ...
   - **バイナリパスの解決 (exePath)**: マウントされるバイナリのホスト側パスは、`--mount-cderun-path` が未指定の場合、`os.Executable()` によって取得されます。ネストされた環境（レベル > 0）で実行されている場合、取得されたパスは `hostContext` を用いて基底ホスト上のパスへとベストエフォートで逆解決されます。
...
`cderun` がネスト呼び出しを行う可能性のあるコンテナを起動する際、ホスト上にスナップショットディレクトリ（例: `/tmp/cderun-snap-<uuid>`）を生成し、コンテナ内の `/run/cderun/` にマウントします。
...
- **削除**: コンテナが終了（または `cderun` プロセスが終了）した際に、実行ホスト上の `/tmp/cderun-snap-<uuid>/` を再帰的に削除します。
