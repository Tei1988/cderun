...
`mounts`, `devices`, `env`, `ports`, `mountTools` などのリスト形式の設定も、スカラ型と同様に **「優先順位の高いソースに値があれば、それより低い優先順位のソースはすべて無視される（完全上書き）」** という挙動になります。

### 「マージ」ではなく「置き換え」 (Replace, not Merge)

cderun は、異なる優先順位レベル間（例：P2 と P4）でリストの内容を統合（マージ）することはありません。

- **例**:
  - **.tools.yaml (P4)**: `env: ["A=1", "B=2"]`
  - **CLI (P2)**: `cderun --env C=3 node`
  - **結果**: コンテナ内の環境変数は `["C=3"]` のみになります。P4 の `A=1`, `B=2` はすべて破棄されます。

**重要な注意点**:
高い優先順位のソースにおいて**明示的な空のリスト**（YAMLでの `[]` や環境変数での空文字列）が指定された場合、それは意図的な「設定の無効化」とみなされ、より低い優先順位のソースの設定をすべて上書きして空にします。

- **例**: `.tools.yaml` (P4) で `mounts: []` と指定した場合、`.cderun.yaml` (P5) に `mounts` 定義があっても、最終的なマウント設定は空になります。
- **環境変数**: `export CDERUN_ENV=""` とした場合も、下位レベル（P4/P5）の環境変数設定をすべて無効化します。

### 同一ソース内での重複排除 (Intra-source Deduplication)

リスト形式の設定（特に環境変数 `env`）において、**同じソース内**（例：CLIフラグのみ、または一つのYAMLファイル内のみ）で同じキーが複数回指定された場合は、**最後に指定された値**が優先されます（Last-one-wins）。

- **例**: `cderun --env A=1 --env A=2 node` を実行した場合、環境変数 `A` の値は `2` になります。

## 特殊な連動ロジック (Transitive Auto-enablement)

一部のオプションは、ユーザーの利便性のために他の設定に基づいて連鎖的に自動有効化されます。これらのロジックは、対象のオプションが P1〜P5 のどのレベルでも**明示的に設定（`nil` 以外）されていない場合にのみ**適用されます。

| トリガー設定 | 自動有効化される設定 | 備考 |
| :--- | :--- | :--- |
| `mountTools` (指定あり) | `mountCderun: true` | ツール実行に必要なバイナリを注入するため |
| `mountAllTools: true` | `mountCderun: true` | 同上 |
| `mountCderun: true` | `mountSocket: true` | cderun の実行にランタイムへのアクセスが必要なため |

### 連動の停止 (Explicit Override)

いずれかの優先順位（P1〜P5）で**明示的に値（`true` または `false`）が指定されている場合**、上記の自動有効化は行われません。

- **例**: `.cderun.yaml` で `mountSocket: false` と設定されている場合、`mountCderun` が有効であってもソケットはマウントされません。
- **例**: コマンドラインで明示的に `--mount-socket=true` を指定した場合は、自動有効化ロジックではなく指定された値が優先されます（結果として同じ `true` になりますが、内部的には自動連動はスキップされます）。
