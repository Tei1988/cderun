...
- **Flag Names**: All supported standard `cderun` options have a corresponding P1 counterpart prefixed with `--cderun-`.
  - **Key P1 Flags**:
    ...
    - **Mounting & Isolation**: `--cderun-mount`, `--cderun-mount-tools`, `--cderun-mount-all-tools`, `--cderun-mount-cderun`, `--cderun-mount-cderun-path`, `--cderun-mount-cderun-socket`, `--cderun-mount-socket`, `--cderun-mount-socket-path`, `--cderun-socket-path`
...
- **Flag Names**:
  - `--tty`, `--interactive`, `--image`, `--network`, `--runtime`, `--oci-runtime`, `--socket-path`, `--mount-socket`, `--mount-socket-path`, `--mount-cderun-socket`, `--env`, `--workdir`, `--mount`, `--mount-cderun`, `--mount-cderun-path`, `--mount-tools`, `--mount-all-tools`, `--prune`, `--remove`, `--config`, `--tool-config`
...
- **Key Variables**:
  - **Configuration & Execution**: `CDERUN_CONFIG`, `CDERUN_TOOL_CONFIG`, `CDERUN_IMAGE`, `CDERUN_RUNTIME`, `CDERUN_OCI_RUNTIME`, `CDERUN_SOCKET_PATH`, `CDERUN_PRUNE`, `CDERUN_REMOVE`, `CDERUN_RESTART`, `CDERUN_STRICT_ENV`, `CDERUN_HANG_TIMEOUT`, `CDERUN_PREFETCH`, `CDERUN_PREFETCH_ALL`
