...
**重要な注意点**:
高い優先順位のソースにおいて**明示的に空のリスト（例: `[]`）**が指定された場合、それは「意図的に空に設定された」とみなされ、より低い優先順位のソースの設定を上書き（消去）します。

- **例**: CLI (P2) やツール設定 (P4) で `env` を空として指定した場合、グローバル設定 (P5) やデフォルト値 (P6) に `env` が定義されていても、それらは無視され、空の環境変数リストで実行されます。
- **意図**: これにより、デフォルトで設定されているマウントや環境変数を、特定の実行やツールにおいて明示的に「なし」にすることが可能になります。
