...
- **配置規則**: 原則として**サブコマンドの後ろ**に配置する必要があります。ただし、`--diagnosis` モードのようにサブコマンドを必要としない実行モードでは、任意の場所に配置可能です。
...
ただし、実装上の制約として、**「空のリスト（`[]`）」を指定した場合は「未設定」とみなされ、より低い優先順位のソースへフォールバックが発生します。** つまり、上位の優先順位で明示的に「空にする（下位の設定を打ち消す）」ことは現在できません。

以前のバージョンでは `mounts` など一部の項目で「マージ（追加）」される挙動がありましたが、現在は一貫して「上書き（またはフォールバック）」に変更されています。
...
4. P1〜P3がなく P4 (`.tools.yaml` の `env`) が**空でなければ**、それを使用し、P5を無視。
5. P4 が空（または未定義）の場合、P5 (`.cderun.yaml` の `defaults.env`) を採用する。

※ 同一の優先順位内（例：複数の `--env` フラグ、またはひとつの設定ファイル内）では、すべての値が合算され、キーが重複する場合は最後に指定された値が優先されます。

## オプションの連鎖有効化 (Transitive Auto-enablement)

特定のオプションが指定されていない場合、他のオプションの状態に基づいて自動的に `true` に設定される「連鎖有効化」ロジックが存在します。

- **`mountCderun` の自動有効化**:
  `--mount-tools` または `--mount-all-tools` (P1-P5) のいずれかが指定されている場合、`mountCderun` が明示的に指定されていなければ自動的に `true` になります。
- **`mountSocket` の自動有効化**:
  `mountCderun` が (自動有効化を含め) `true` になっている場合、`mountSocket` が明示的に指定されていなければ自動的に `true` になります。

※ P1〜P5のいずれかで明示的に `false` が設定されている場合は、この自動有効化は行われません。
