...
詳細な動作メカニズムについては [引数解析](./argument-parsing.md) を参照してください。
...
`tty` オプションが以下の複数の場所で指定されている場合を考えます。
...
`mounts` 設定の解決プロセスをステップバイステップで追跡します。

**設定状況:**

- **P5 (.cderun.yaml)**: `/home/user/common:/common` (共通マウント)
- **P4 (.tools.yaml)**: `node` ツールに `src:/app` (ツール固有マウント)
- **P2 (CLI)**: `--mount type=bind,source=/data,target=/data`

**解決ステップ:**

1. **P1 (Internal Override)**: 指定なし → 次へ
2. **P2 (CLI Flags)**: `--mount ...` が指定されている。
   - **決定**: この時点で P2 が「勝利」します。
   - **結果**: `mounts` は `/data:/data` の1件のみとなります。P4 や P5 の設定は**マージされず、破棄されます。**

もし P2 で指定がなかった場合：

1. **P3 (Env Var)**: `CDERUN_MOUNT` なし → 次へ
2. **P4 (Tool Config)**: `node` に `src:/app` が定義されている。
   - **決定**: P4 が「勝利」します。
   - **結果**: `mounts` は `src:/app` の1件となります。P5 の共通マウントは無視されます。
...
**重要な注意点**:
ある優先順位レベルにおいて設定が**明示的に（空リスト `[]` を含めて）**記述されている場合、そのレベルが「勝利」し、それより低い優先順位の設定はすべて無視されます。

- **明示的な上書き**: `.tools.yaml` (P4) で `mounts: []` と記述した場合、これは「マウントを一切行わない」という明示的な設定となり、`.cderun.yaml` (P5) に `mounts` の定義があっても無視されます。
- **フォールバック**: そのレベルで設定自体が「未定義（`nil`）」である場合にのみ、次の優先順位レベルへ探索が進みます。
