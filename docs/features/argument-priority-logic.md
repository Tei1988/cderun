...
いずれかの優先順位（P1〜P5）で明示的に `false` が指定されている場合、上記の自動有効化は行われません。

- **例**: `.cderun.yaml` で `mountSocket: false` と設定されている場合、`mountCderun` が有効であってもソケットはマウントされません。これにより、セキュリティ上の理由でソケット露出を避けたい場合などの意図を尊重します。
