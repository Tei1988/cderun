...
**動作イメージ（ランタイムや指定パスの設定に応じて柔軟に解決されます）**:

```bash
# .tools.yamlに node, python, gemini-cli が定義されている場合
# ※ 以下の <socket-path>, <mount-socket-path>, <mount-cderun-path> には解決された値が入ります
docker run --rm \
  --mount type=bind,source=<socket-path>,target=<mount-socket-path> \
  --mount type=bind,source=<mount-cderun-path>,target=/usr/local/bin/cderun,readonly \
  --mount type=bind,source=<mount-cderun-path>,target=/usr/local/bin/node,readonly \
  --mount type=bind,source=<mount-cderun-path>,target=/usr/local/bin/python,readonly \
  --mount type=bind,source=<mount-cderun-path>,target=/usr/local/bin/gemini-cli,readonly \
  alpine:latest
```
...
**動作イメージ(実際はランタイムAPIで実現され、設定に応じたパスがマウントされます)**:
