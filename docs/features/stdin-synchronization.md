...
この仕組みにより、`echo "test" | cderun cat` のようなコマンドにおいて、Dockerランタイム側の問題で終了が遅延する場合でも、2秒以内に確実に制御が戻ることが保証されます。また、`cderun -it alpine sh` のような対話型実行では、`--tty` フラグによってこの自動終了ロジックが無効化され、ユーザーフレンドリーな動作が維持されます。
