...
この問題に対処するため、`cderun` は非インタラクティブ実行時に以下の自動終了ロジックを実装しています。
...
4. **インタラクティブ性の維持**: この自動終了ロジックは、インタラクティブモード（`--interactive` または `-i`）が有効かつ、ホストの標準入力が端末である場合は**無効**になります。これにより、インタラクティブなシェルや長時間実行される UI アプリケーションが誤って終了されるのを防ぎます。
...
`cderun` は、ホストの標準入力（STDIN）が端末であるか、およびインタラクティブモードが有効であるかを確認します。

1. **`--interactive` または `-i` フラグ**: コンテナの標準入力を開いたままにするかどうかを制御します。
2. **ホスト標準入力の状態**: ホストの標準入力が端末（TTY）かどうかを指します。

### 挙動

IO 操作が完了した後（STDIN のコピーと出力ストリームの両方が EOF に達した後）、`cderun` はコンテナが自然に終了するのを待ちます。

- **ハングタイムアウト**: 現在の実装では、CI環境等の安定性を考慮し、一律で **2秒**（環境変数 `CDERUN_HANG_TIMEOUT` で変更可能）の猶予期間を設けています。
- IO 完了後、猶予期間内にコンテナが終了しない場合、かつ「インタラクティブモードが有効かつホスト標準入力が端末」という条件を満たさない場合、`cderun` は `SIGKILL` を送信し、プロセスを確実に終了させます。

この仕組みにより、`echo "test" | cderun cat` のようなコマンドにおいて、Dockerランタイム側の問題で終了が遅延する場合でも、2秒以内に確実に制御が戻ることが保証されます。また、`cderun -it alpine sh` のような対話型実行では、この自動終了ロジックが無効化され、ユーザーフレンドリーな動作が維持されます。
