...
また、Docker デーモン側の実装（バージョン 29.1.5 など）によっては、データのコピー直後に `CloseWrite()` を呼び出すと、EOF の処理がデータの読み取りより優先されたり、接続が不安定になったりする場合があることが確認されています。
...
### 3. 書き込み終了前の猶予期間 (CloseWrite Grace)

一部の Docker バージョンにおける EOF 処理の不安定さを回避するため、STDIN のコピー完了から `CloseWrite()` を実行するまでに、短い猶予期間（デフォルト `1s`）が設けられています。これにより、デーモンがデータを確実に処理してから EOF を受け取ることが保証されます。
