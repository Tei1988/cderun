...
2. **ホスト標準入力の状態**: ホストの標準入力が端末かどうかを指します。これは**ハングタイムアウト**の長さを決定するための情報として使用されます。
...
- **ハングタイムアウト**: 現在の実装では、CI環境等の安定性を考慮し、ホスト標準入力の状態に関わらず一律で **2秒**（`CDERUN_HANG_TIMEOUT` で変更可能）の猶予期間を設けています。
