...
4. **インタラクティブ性の維持**: この自動終了ロジックは、ホストの標準入力が端末であり、かつインタラクティブモード（`--interactive` または `-i`）が有効である場合にのみ**無効**になります。それ以外の場合（標準入力がパイプである場合や、非インタラクティブ実行の場合）は、一律でハングタイムアウトが適用されます。これにより、インタラクティブなシェルが誤って終了されるのを防ぎつつ、バッチ処理での確実な終了を保証します。
...
`cderun` は、以下の条件に基づいて自動終了（SIGKILL）を適用するかどうかを判定します。

- **無効化される条件**: 「ホストの標準入力が端末である」かつ「インタラクティブモードが有効である」場合。
- **適用される条件**: 上記以外（標準入力がパイプ/ファイルである、または非インタラクティブモードである場合）。

### 挙動

IO 操作が完了した後（STDIN のコピーと出力ストリームの両方が EOF に達した後）、`cderun` はコンテナが自然に終了するのを待ちます。

- **ハングタイムアウト**: 現在の実装では、CI環境等の安定性を考慮し、ホスト標準入力の状態に関わらず一律で **2秒**（環境変数 `CDERUN_HANG_TIMEOUT` で変更可能）の猶予期間を設けています。
