...
- **`CDERUN_HANG_TIMEOUT`**: 非インタラクティブまたは非TTYセッションにおける、I/O完了後の終了猶予時間（デフォルト: `2s`）。ホスト入力がTTYでないか、コンテナが非インタラクティブな場合にのみ適用されます。タイムアウト経過後もコンテナが実行中の場合、`SIGKILL` による強制終了を試行します。
