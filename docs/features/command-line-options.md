...
`cderun` は多数の環境変数による設定をサポートしています（優先順位 P3）。詳細は [引数・設定優先順位](./argument-priority-logic.md) を参照してください。

主要な特殊変数：

- **`CDERUN_HANG_TIMEOUT`**: I/O完了後、コンテナが終了するのを待機するタイムアウト時間（デフォルト: `2s`）。`500ms`, `5s` のようなGoのDuration形式で指定します。主にCI環境でのハング対策に使用されます。タイムアウト経過時、コンテナが既に終了していれば即座に復帰し、実行中であれば `SIGKILL` による強制終了を試行します。
- **`CDERUN_CONFIG`**: cderun自体の設定ファイル（`.cderun.yaml`）のパスを指定します。
- **`CDERUN_TOOL_CONFIG`**: ツール設定ファイル（`.tools.yaml`）のパスを指定します。
