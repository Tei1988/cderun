...
### 未知のディレクティブの扱い (Handling Unknown Directives)

現在、`cderun` は未知のディレクティブやタイポ（例: `{{TYPO}}`, `{{HOM}}`）に遭遇した場合、それらを解決せずにリテラル文字列としてそのまま維持します。

これは、設定値として意図的に `{{...}}` 形式の文字列（例: Go template や Helm チャートの変数）を渡したいユースケースを許容するための現在の挙動です。しかし、タイポによるミスを防ぐため、将来的なリリースでは未知のディレクティブをエラーとして扱うように変更される可能性があります（参考: [TODO T11](../../.agent/todo.md)）。

### ネストした式の解決 (Nested Expressions)

`cderun` は式のネストをサポートしています。解析は内側から外側へと再帰的に行われます。

**例:**

- **デフォルト値へのマジックワード指定**:
  `{{env:DATA_DIR:-{{HOME}}/data}}`
  `DATA_DIR` が未設定の場合、ホームディレクトリ配下の `data` ディレクトリが採用されます。
- **動的なキーの決定**:
  `{{env:API_KEY_{{env:STAGE:-DEV}}}}`
  `STAGE` が `PROD` であれば `{{env:API_KEY_PROD}}` が、未指定なら `{{env:API_KEY_DEV}}` が評価されます。
...
解決プロセス中に制約違反（セキュリティ違反、ファイル不在、ファイルサイズ超過など）が検出された場合、解決エンジン（`ExpressionResolver`）は即座にエラーを内部に保持（`setError`）し、以降のすべての動作をロックします。

1. **即時のロック**: 一度エラーが発生すると、その resolver インスタンスは「汚染（Dirty）」状態となり、以降のすべての解決リクエストに対して処理を試行せず、元の入力値をそのまま返します。
2. **不完全な解決の防止**: これにより、例えば複数の `{{...}}` 式を含む文字列において、一部だけが解決され残りが未解決のまま中途半端な値がコンテナに渡されるといった、危険な状態を防ぎます。
3. **確実なエラー伝搬**: スライスやマップのように大量の項目を解決する場合でも、途中の1箇所で起きたエラーが最終的に呼び出し元まで確実に伝わり、システム全体を安全に停止（Fail-fast）させることができます。

この「スティッキー（粘着性）」な特性により、一貫性と安全性が保証されます。
...
`.git` や `package.json` が存在するディレクトリ（プロジェクトルート）を自動的に探し出し、その絶対パスを使ってファイルをマウントできます。 なお、Nested 実行環境（コンテナ内）でマウント情報がある場合、見つかったディレクトリのパスはホスト側の絶対パスへ逆解決（host path へ再マッピング）されます。自分がプロジェクト内のどの階層（`src/` や `tests/` 等）にいても、常に正しいソースを参照できます。
