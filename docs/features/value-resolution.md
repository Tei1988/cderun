...
| `{{find_dir:<名前>}}` | 指定されたファイルまたはディレクトリ `<名前>` を `{{file:...}}` と同じルールで上方探索し、それが発見されたディレクトリへの**絶対パス**で置換します。見つからない場合はエラーとなります。 |
...
`.git` や `package.json` が存在するディレクトリ（プロジェクトルート）を自動的に探し出し、その絶対パスを元にファイルをマウントできます。自分がプロジェクト内のどの階層（`src/` や `tests/` 等）にいても、常に正しいソースを参照できます。
...
> [!IMPORTANT]
> `find_dir` はそれ自体で発見されたディレクトリの**絶対パス**を返します。
> `source: "{{PWD}}/{{find_dir:.git}}"` のように記述すると、カレントディレクトリの下にさらに絶対パスが続くような不正なパス構成になるため、単体で `source: "{{find_dir:.git}}"` と記述してください。
