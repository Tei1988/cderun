...
設定ファイル（YAML）から読み込まれた値が構造体やマップである場合、cderun はその内部を再帰的に走査して解決を適用します。これにより、ネストされた設定の奥深くにある式も正しく評価されます。

- **文字列 (`string`)**: 式の展開、チルダ展開、相対パスの絶対パス化が適用されます。
- **スライス (`[]any`)**: 各要素に対して再帰的に解決が実行されます。
- **マップ (`map[string]any`)**: 各値に対して再帰的に解決が実行されます。
