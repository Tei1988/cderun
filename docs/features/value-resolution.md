...
cderun では、設定ファイル内の値やコマンドライン引数で指定された値が、実行前にいくつかの変換プロセスを経て解釈されます。これにより、環境に応じた柔軟な設定や動的な値の埋め込みが可能になります。

これらの解決ルールは、**設定ファイル内の値**と**コマンドライン引数の値**の両方に適用されます。

## 解決の仕組み (How it Works)

cderun の内部では、単純な文字列だけでなく、複雑なデータ構造に対しても一貫した解決ロジックが適用されます。

### 1. 再帰的解決 (Recursive Resolution)

cderun は、設定ファイルから読み込まれた `string`, `[]any`, `map[string]any` 型のデータを再帰的に走査して解決を適用します。これにより、ネストされた設定の奥深くにある式も正しく評価されます。

- **`string` (文字列)**: 式の展開、チルダ展開、相対パスの絶対パス化が適用されます。
- **`[]any` (スライス)**: 各要素に対して再帰的に解決が実行されます。
- **`map[string]any` (マップ)**: 各値に対して再帰的に解決が実行されます。

### 2. スティッキーエラー (Sticky Error) パターン

解決プロセス中に、セキュリティ制約違反（例：親ディレクトリへの脱出）やファイル不在などのエラーが一つでも検出された場合、解決エンジン（`ExpressionResolver`）は「エラー状態」に移行します。

- **安全なフォールバック**: エラー発生以降のすべての解決リクエストは、元の値をそのまま（未解決のまま）返します。これにより、不完全または危険な解決結果が使用されるのを防ぎます。
- **確実な中断**: 解決の最後にエラーチェックが行われ、一つでも違反があればプロセス全体が安全に停止します。
...
| ディレクティブ | 説明 |
| :--- | :--- |
| `{{file:<ファイル名>}}` | 指定された `<ファイル名>` の内容を読み込み、その値で置換します。ファイルは、**カレントディレクトリからルートディレクトリに向かって順次親ディレクトリを遡り（上方探索）**、それでも見つからない場合は `~/.config/cderun/`, `/etc/cderun/`, `/run/cderun/` を検索します。見つかったファイル内容の前後の空白・改行は除去されます。ファイルが見つからない場合や、絶対パス・`..` を含むファイル名が指定された場合はエラーとなります。また、セキュリティ上の理由から **1MB (1,048,576 bytes)** を超えるファイルは読み込めません。 |
...
マジックワード（`{{HOME}}` 等）やチルダ（`~`）を起点とするパス解決では、利便性と安全性を両立させるため、以下のルールが適用されます。
...
`.git` や `package.json` が存在するディレクトリ（プロジェクトルート）を自動的に探し出し、そこからの相対パスでファイルをマウントできます。自分がプロジェクト内のどの階層（`src/` や `tests/` 等）にいても、常に正しいソースを参照できます。
...
#### 4. ネスト実行における基底ホストパスの参照 (`{{BASE_...}}`)

ネスト実行環境（コンテナ内）では、`{{HOME}}` はコンテナ内のホーム（例：`/root`）に展開されますが、`{{BASE_HOME}}` を使うことで基底ホストのパスを指し示すことができます。

```yaml
# .tools.yaml
# コンテナ内からさらに別のコンテナを起動する際、
# 基底ホスト側の設定ファイルをマウントしたい場合に有効
clasp:
  mounts:
    - type: bind
      source: "{{BASE_HOME}}/.config/clasp"
      target: "/root/.config/clasp"
```
