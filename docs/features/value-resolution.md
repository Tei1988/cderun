...
##### マジックワードの遷移例 (Nesting Level Example)

ネストレベルに応じて、各マジックワードが指し示すパスは以下のように遷移します。
...
#### 3. ネストされた式 (Nested Expressions)

cderunの式解決は再帰的に行われます。ある式の解決結果（例: `{{env:...}}` のデフォルト値部分）の中に別の式が含まれている場合、それらも解決の対象となります。

- **例**: `{{env:TAG:-{{file:.version}}}}`
  1. `TAG` 環境変数が存在すればその値を使用します。
  2. 存在しない場合、`.version` ファイルの内容を読み込み、その値をデフォルト値として使用します。
- **例**: `{{env:DIR:-{{HOME}}/projects}}`
  1. `DIR` 環境変数が存在すればその値を使用します。
  2. 存在しない場合、ホームディレクトリ配下の `projects` ディレクトリへのパスとして解決されます。

この機能により、環境変数、ファイル、マジックワードを組み合わせた非常に柔軟な動的設定が可能になります。

#### 4. 未知のディレクティブの扱い

`{{...}}` 内のコンテンツがマジックワードや既知のディレクティブのいずれにも該当しない場合、現在はタイポや意図しない挙動を防ぐための厳格なバリデーションへの移行期間として、**そのままリテラル文字列として残されます**（例: `{{TYPO}}` → `{{TYPO}}`）。

将来のバージョン（**バージョン 2.0 以降を予定**）では、安全性の向上のため、未知のディレクティブはエラーとして処理される（実行が中断される）ようになります。現在は移行期間として、未知の式をそのまま通しますが、タイポに注意してください。
...
**ユーザーへの影響**:
例えば、`.tools.yaml` の `env` リストに5つの項目があり、そのうち2番目の項目の `{{file:...}}` がファイル不在でエラーになった場合、2番目以降（3番目から5番目）の式解決はすべてスキップされ、元の未解決な文字列がそのまま返されます。最終的に、cderunは最初に発生した「ファイル不在」エラーを報告して停止します。
