...
### 3. 作業ディレクトリの厳格な検証 (Workdir Traversal Defenses)

`ValidateWorkdir` によるコンテナ作業ディレクトリの検証は、作業ディレクトリが明示的に指定されている場合にのみ実施され、以下のように厳格に制御されます。

- **`..` の完全排除**: 親ディレクトリ参照（`..`）を含むパスは、いかなる場合でも完全に拒否されます。これにより、意図しないパスの難読化や、コンテナのルートファイルシステム脱出攻撃を防御します。
...
- **実ファイルシステム動的パス解決**: `RealFileSystem` は、テスト実行や動的なカレントディレクトリ変更（`os.Chdir` など）に伴う誤ったマッピング結果やOSエラーの継続を防止するため、`Getwd` や `UserHomeDir` に関するプロセス永続キャッシュを**あえて行わず**、解決要求の都度、最新の状態をOSから動的に取得します。
