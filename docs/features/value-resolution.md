...
| ディレクティブ | 説明 |
| :--- | :--- |
| `{{file:<ファイル名>}}` | 指定された `<ファイル名>` の内容を読み込み、その値で置換します。ファイルは、カレントディレクトリからルートディレクトリ、および `~/.config/cderun/`, `/etc/cderun/`, `/run/cderun/` を順に上方探索して検索されます。見つかったファイル内容の前後の空白・改行は除去されます。最大ファイルサイズは **1MB** です。ファイルが見つからない場合や、絶対パス・`..` を含むファイル名、あるいはディレクトリ区切り文字 (`/`, `\`) を含むファイル名が指定された場合はエラーとなります。 |
| `{{find_dir:<名前>}}` | 指定されたファイルまたはディレクトリ `<名前>` を `{{file:...}}` と同じルールで上方探索し、それが発見されたディレクトリへの**カレントディレクトリ（`{{PWD}}`）からの相対パス**で置換します。見つからない場合はエラーとなります。`file` ディレクティブと同様、ディレクトリ区切り文字を含む名前は指定できません。 |
| `{{env:<環境変数名>}}` | 実行環境の環境変数 `<環境変数名>` の値に置換されます。環境変数が存在しない場合は空文字列に置換されます。また、`{{env:KEY:-default}}` の形式で、環境変数が未設定または空の場合のデフォルト値を指定できます。 |

### セキュリティ制約

`cderun` は、設定値や式の解決において以下のセキュリティチェックを強制します。

#### 1. 式（Directives）の制約

`{{file:...}}` および `{{find_dir:...}}` ディレクティブには、ディレクトリトラバーサルなどの攻撃を防止するため、以下の厳格な制約があります。

- **絶対パスの禁止**: `/etc/passwd` や `C:\windows\system32\...` のような絶対パスは指定できません。
- **親ディレクトリ参照の禁止**: `..` セグメントを含むパス（例: `../../secret`）は指定できません。
- **ディレクトリ区切り文字の禁止**: `/` や `\` を含むパスは指定できません。これにより、探索範囲をプロジェクト境界内の単純なファイル名に限定します。
- **ファイルサイズ制限**: `{{file:...}}` で読み込めるファイルは **1MB (MaxDirectiveFileSize)** までです。

これらの制約は、Go 1.20 で導入された `filepath.IsLocal` 関数を使用して厳密に検証されます。

#### 2. 文字列の安全検証 (validatePathChars)

解決されたすべてのパス（イメージ名、ユーザー名、ネットワーク名、ホスト名、作業ディレクトリ、エントリポイントなど）に対し、以下の文字が含まれていないか検証されます。

- **ヌルバイト (`\x00`)**
- **制御文字 (ASCII < 32 または 127)**

これらが検出された場合、インジェクション攻撃の可能性があるとみなされ、実行が中断されます。
