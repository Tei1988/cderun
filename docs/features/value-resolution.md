...
#### 複数アンカーの例 (Multiple Anchors Scenarios)

`{{HOME}}` が `/home/user`、`{{PWD}}` が `/home/user/work` と解決された場合：
...
1. **エラーとなるケース**: コンテンツが「マジックワード風（すべて大文字英数字とアンダースコア）」または「ディレクティブ風（`:` を含む）」である場合、未知の指定として**エラー**となり、実行が中断されます。
   - **例**: `{{HOM}}` (タイポ), `{{unknown:key}}` (未知のディレクティブ)
...
この記法により、cderunの解決エンジンによる置換を明示的に回避できます。
...
1. **安全なフォールバック**: エラー発生以降のすべての解決リクエストは、元の値をそのまま（未解決のまま）返します。これにより、不完全または危険な解決結果が使用されるのを防ぎます。一度エラー状態に陥ると、たとえ後続の式が正しくても解決は行われません。
...
#### 4. ネストされた式 (Nested Expressions)

式の中にさらに別の式を含めることができます。内側の式が先に解決され、その結果を使って外側の式が評価されます。

```bash
# 環境変数 VAR_NAME に格納された名前の環境変数を参照する
cderun --env "VALUE={{env:{{env:VAR_NAME}}}}" my-tool

# ディレクトリが見つからない場合のデフォルト値としてホームディレクトリを指定する
cderun --env "CONFIG_DIR={{env:CONF_DIR:-{{HOME}}/default_conf}}" my-tool
```
