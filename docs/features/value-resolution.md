...
| ディレクティブ | 説明 |
| :--- | :--- |
| `{{file:<ファイル名>}}` | 指定された `<ファイル名>` の内容を読み込み、その値で置換します。ファイルは、カレントディレクトリからルートディレクトリ、および `~/.config/cderun/`, `/etc/cderun/`, `/run/cderun/` を順に上方探索して検索されます。見つかったファイル内容の前後の空白・改行は除去されます。ファイルが見つからない場合や、絶対パス・`..` を含むファイル名が指定された場合はエラーとなります。 |
| `{{find_dir:<名前>}}` | 指定されたファイルまたはディレクトリ `<名前>` を `{{file:...}}` と同じルールで上方探索し、それが発見されたディレクトリへの**カレントディレクトリ（`{{PWD}}`）からの相対パス**で置換します。見つからない場合はエラーとなります。 |
| `{{env:<環境変数名>}}` | 実行環境（実行ホスト）の環境変数 `<環境変数名>` の値に置換されます。環境変数が存在しない場合は空文字列に置換されます。また、`{{env:KEY:-default}}` の形式で、環境変数が未設定または空の場合のデフォルト値を指定できます。 |
...
解決プロセス中にこれらの制約違反（絶対パスの指定や `..` の使用など）が検出された場合、解決エンジン（`ExpressionResolver`）は即座にエラーを内部に保持（`setError`）します。

1. **連鎖的な停止**: 以降のすべての解決リクエストは、既にエラーが発生しているため、元の値をそのまま返します。
2. **確実な中断**: 最終的な解決結果を取得する際、呼び出し元は `Error()` メソッドを通じて最初の違反内容を確認し、エラーがあれば実行を即座に中断（Fatal Error）させます。
3. **部分的な成功の防止**: この仕組みにより、複数の値が含まれるスライスやマップの解決において、一部の不正な値が「静かに無視」されるのを防ぎ、システム全体の安全性を確保します。
