...
| ディレクティブ | 説明 |
| :--- | :--- |
| `{{file:<ファイル名>}}` | 指定された `<ファイル名>` の内容を読み込み、その値で置換します。ファイルは、カレントディレクトリからルートディレクトリ、および `~/.config/cderun/`, `/etc/cderun/`, `/run/cderun/` を順に上方探索して検索されます。見つかったファイル内容の前後の空白・改行は除去されます。ファイルが見からない場合や、絶対パス・`..` を含むファイル名が指定された場合はエラーとなります。また、セキュリティ上の理由から **1MB (1,048,576 bytes)** を超えるファイルは読み込めません。 |
...
> [!IMPORTANT]
> `source: "{{PWD}}/{{find_dir:.git}}"` のように記述すると、`.git` がカレントディレクトリ（`PWD`）より上位にある場合に「アンカー境界検証」によってトラバーサルエラーが発生します。`find_dir` はそれ自体で適切な相対パス（または絶対パス）を返すため、単体で `source: "{{find_dir:.git}}"` と記述するのが正しい方法です。
...
`mounts` の `source` や `devices` の host パスなど、ホスト側のパスを指す設定値に `./` や `../` で始まる相対パスが記述されている場合、そのパスは以下の基準で絶対パスへ変換されます。
