...
cderunの内部では、文字列だけでなく、スライス（配列）やマップ（オブジェクト）に対しても再帰的に値の解決が適用されます。これにより、ネストされた設定値の中にある `{{...}}` 式も正しく評価されます。
...
これらの制約に違反した（絶対パスや `..` が検出された）場合、エラーは解決エンジンに記録され、呼び出し元へ伝搬されます。呼び出し元はこのエラーを検知して実行を中断するなどの適切な処理を行います。
