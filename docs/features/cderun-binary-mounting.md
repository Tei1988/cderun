...
> **Note**: 設定ファイル（`.cderun.yaml` や `.tools.yaml`）は自動的にはマウントされません。コンテナ内からcderunを使用する場合、必要な設定は環境変数経由で渡すか、マウントを使用して設定ファイルをコンテナ内の検索パス（例：カレントディレクトリ）に配置する必要があります。
...
1. **Completed**: 基本的なcderunバイナリおよびソケットのマウントが実装済み。
