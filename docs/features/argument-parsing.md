...
1. サブコマンドの後ろ（ポリグロットモードの場合は実行ファイル名以降）の引数リストを走査中に `--` が見つかった場合、**それ以降の引数のスキャンおよびホイスト処理を即座に終了（打ち切り）** します。
2. `--` 以降にある引数は、たとえ `--cderun-` プレフィックスを持っていたとしてもホイストされず、そのままリテラルのパススルー引数としてコンテナコマンドへ引き渡されます。
...
ホイスト処理は `--` を見つけた時点で処理を打ち切るため、この時の実際のコンテナ実行は `echo --cderun-tty` となります。
