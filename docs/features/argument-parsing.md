...
1.  **サブコマンドの特定**: 最初の非フラグ引数をサブコマンドとして特定します。この際、既知の `cderun` 標準フラグ（P2）とその引数は適切にスキップされます。
2.  **P1フラグの収集**: サブコマンド以降の引数リストをスキャンし、`--cderun-` で始まるフラグを収集します。
3.  **引数値のハンドリング**: フラグが値を必要とする場合（例: `--cderun-image node:20-alpine` で `=` を使わない形式）、次の引数もセットで収集・移動されます。
4.  **再構成**: 収集された P1 フラグを `cderun` 直後（サブコマンドの前）に挿入し、残りの引数（サブコマンドとパススルー引数）をその後に配置します。
