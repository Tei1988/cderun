...
- **\<subcommand\>**: 最初の非フラグ引数（Cobraのフラグ定義に合致しないもの）。`.tools.yaml` から設定を読み込むための**キー**としてのみ使用される。コンテナの `CMD` には含まれない。
- **[passthrough-args]**: サブコマンド以降の全引数。コンテナ内で実行されるコマンドの引数として渡される。
  - `--cderun-` フラグ（P1）が含まれる場合は前方にホイストされ、`cderun` 自体の設定として解釈されます。
  - **重要**: サブコマンドの後ろに配置された標準フラグ（例: `--tty`）は、ホイストの対象とならず、**パススルー引数としてそのままコンテナ内のコマンドに渡されます。**
...
> **注意**: Wrapper Mode では、バリデーションにより P1 フラグをサブコマンドの**前**に置くことは禁止されており、エラーとなります。P1 フラグは常にサブコマンドの後ろに配置してください。
...
1. **サブコマンドの特定**: 登録済みの標準フラグ（P2）とその引数をスキップしながら、最初の非フラグ引数を「サブコマンド（ルックアップキー）」として特定します。これにより、フラグの値として渡された引数を誤ってサブコマンドと判定することを防ぎます。
2. **P1フラグの収集**: サブコマンドの境界より後ろにある引数リストをスキャンし、`--cderun-` で始まる内部オーバーライドフラグ（P1）をすべて抽出します。
3. **引数値のハンドリング**: 抽出した P1 フラグが値を必要とする場合（例: `--cderun-image` は後続のイメージ名が必要）、内部のフラグ定義を参照して次の引数もセットで抽出・ホイストの対象とします。
4. **再構成**: 抽出された P1 フラグ群をサブコマンドの直前に挿入します。これにより、Cobra パーサーはこれらを `cderun` コマンド自体のフラグとしてパースできるようになります。
