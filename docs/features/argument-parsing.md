...
1. **サブコマンドの特定**: 最初の非フラグ引数をサブコマンドとして特定します。このプロセスは、標準的な `Wrapper Mode` と `Symlink Mode (Polyglot)` で異なります。
   - **Wrapper Mode**: 最初の引数（`cderun`）をスキップし、それ以降で「`-` で始まらない最初の文字列」をサブコマンド境界として探索します。
   - **スキップロジック**: 既知の `cderun` 標準フラグ（P2）が値を必要とする場合（例: `--image <name>`）、その値もスキップして境界を誤認しないように制御されます。
   - **Symlink Mode**: 実行ファイル名自体（例: `node`）がサブコマンドとして扱われるため、引数リストのインデックス 0 が常に境界となります。
2. **P1フラグの収集**: サブコマンドの境界以降の全引数をスキャンし、`--cderun-` で始まるフラグを抽出します。
