...
3. **終了後の出力同期**: コンテナが終了した後、残りの出力を読み取るための短い猶予期間（`5s`）が設けられています。
4. **猶予期間 (Hang Timeout)**: IO が完了したにもかかわらず（または IO 待ちの状態で）、一定の猶予期間内にコンテナが終了しない場合、ハングが発生したとみなします。
5. **強制終了**: タイムアウト経過後、コンテナがまだ実行中であれば `SIGKILL` を送信して強制終了し、CLI の制御をホストに戻します。
