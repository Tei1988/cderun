...
コンテナを作成する前に、`Env` 配列内の各要素をスキャンし、`=` を含まない要素（キーのみの指定）については、抽象ファイルシステム抽象層を通じて `fs.LookupEnv(key)` を呼び出すことで、存在判定を伴う環境変数のルックアップを行います。

これにより、厳密モード（Strict Mode）において「ホスト環境で変数が未設定（存在しない）」状態と「空文字列として明示的に設定されている」状態を正確に識別することができます。解決された値は、最終的に `KEY=value` の形式へと正規化され、コンテナランタイムの作成APIに渡されます。
