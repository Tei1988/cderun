...
- **Podman**: Docker 互換の API を使用。Docker クライアントライブラリを共通の基盤として利用し、Podman の Unix ソケット経由で接続。Podman のシステムサービスとの通信の安定性を向上させるため、以下の設定を適用しています。
  - **カスタムトランスポート**: Unix ソケット専用の `http.Transport` を使用し、`DisableKeepAlives: true` を設定することで、接続の再利用に起因する EOF エラーを回避しています。
  - **API バージョン固定**: 互換性の安定のため、API バージョンを `1.41`（Podman 4.0 以上に対応）に固定しています。
- **イメージプルのリトライ**: ネットワークの不安定さやレート制限（`toomanyrequests` 等）に対応するため、指数バックオフを伴うリトライロジックを実装しています。
  - **リトライ回数**: `--pull-max-retries` で設定可能（デフォルト: 3回）。
  - **バックオフ基底時間**: `--pull-backoff-base` で設定可能（デフォルト: 1秒）。
