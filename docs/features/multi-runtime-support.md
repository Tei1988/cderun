...
- **containerd (Experimental)**:
  ネイティブの containerd API (gRPC) を直接利用したサポートが試験的に導入されています。
  - **目的**: Docker/Podman デーモンを経由しない、より軽量なコンテナ実行の実現。
  - **制限事項**: 現時点では **`host` ネットワークのみ**をサポートしています。ブリッジネットワーク、ポートマッピング（`--publish`）、およびコンテナの露出（`--expose`）などの高度な機能を使用すると、実行時に「未対応」のエラーが発生します。
- **nerdctl (Backlog)**:
