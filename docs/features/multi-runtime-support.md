...
- **containerd (Planned / In-progress)**:
  ...
  - **開発ステータス**: 内部的な設定バリデータ（`resolver.go`）では将来の対応を見越して `containerd` を有効な値として受け入れますが、**ランタイムエンジン本体（`internal/runtime`）の実装が完了していないため、指定しても実行時に「unsupported runtime」エラーとなります。**
