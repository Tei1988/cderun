...
通常のツール利用においてノイズとなる警告を減らすため、以下のイベントは `DEBUG` レベルでログ出力されます：

1. **強制終了イベント**: IO 完了後にコンテナが自然に終了しなかった場合の強制終了（SIGKILL）および、それに伴う後続の待機エラー。これは Docker 29 等のランタイム側の挙動に起因するものであり、`cderun` が適切にハンドルしているため、通常実行では警告を表示しません。
2. **ネスト実行時のパス解決失敗**: ネスト実行時、ホスト側の `cderun` バイナリパスをコンテナ内から特定する際の解決エラー。ベストエフォートでの解決を試みるため、失敗してもフォールバック処理が行われる場合は `DEBUG` レベルで出力されます。
