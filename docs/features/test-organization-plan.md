...
| パッケージ | カバレッジ率 | 備考 |
| :--- | :--- | :--- |
| `internal/command` | 89.1% | コアロジック、フラグ解析、ドライラン、ネスト実行等は良好。 |
| `internal/config` | 83.0% | 設定の読み込み、マージ、Expression解決、パス解決等は良好。 |
| `internal/logging` | 77.3% | ロギングシステムのリデザインに伴い、テストが大幅に拡充された。 |
| `internal/runtime` | 23.5% | リトライロジックのテストが追加された。実機依存部はモックで対応。 |
| **Total** | **76.7%** | 全体として高いカバレッジを維持。ランタイム依存部を除けば極めて高い。 |
