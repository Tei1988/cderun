...
| パッケージ | カバレッジ率 | 備考 |
| :--- | :--- | :--- |
| `internal/command` | 89.5% | コアロジック、フラグ解析、ドライラン、ネスト実行等は良好。 |
...
| 直接コンテナ実行 | `root_test.go` (MockRuntime), `integration_test.go`, `config_test.go` | 良好 |
...
| README生成 | - | 対象外 (開発フロー) |
| Nested Execution | `snapshot_test.go`, `path_test.go`, `scenario_nested_test.go` | 良好 (スナップショット、パス解決、シナリオ検証) |
