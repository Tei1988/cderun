...
`cderun` は以下の 6 つの優先順位（P1 が最高、P6 が最低）に基づいて設定を解決します。

- **P1: CDERUN 内部オーバーライド**: サブコマンドの後ろに指定する `--cderun-` プレフィックス付きフラグ。
- **P2: CLI 標準フラグ**: ユーザーが明示的に指定した `--tty` や `--env` などのフラグ。
- **P3: 環境変数**: `CDERUN_` で始まる環境変数。
- **P4: ツール固有設定**: `.tools.yaml` 内のツールごとの設定。
- **P5: グローバルデフォルト**: `.cderun.yaml` の `defaults` ブロック。
- **P6: ハードコードされたデフォルト**: プログラム内部で定義された最終的なフォールバック値。
