...
### Phase 1: コア機能（必須）
基本的なコンテナ実行機能を実装（Docker Engine対象）

### Phase 2: 設定管理
設定ファイルとイメージマッピング

### Phase 3: 高度な機能
環境変数、マウント機能など（Docker環境でツールが動作することを優先）

### Phase 4: 利便性向上
ドライラン、ログなど

### Phase 5: マルチランタイムサポート
Podman 等の他のコンテナランタイムへの対応
...
**目的**: Podman API を使用してランタイムインターフェースを実装。

**実装内容**:
- `internal/runtime/podman.go` のスタブを実際の Podman 接続ロジックに置き換える。
- Docker 互換 API を使用して、Podman 環境でも同様の操作ができるようにする。

**完了条件**:
- `--runtime podman` を指定した際に、Podman 経由でコンテナが実行されること。
