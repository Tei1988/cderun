...
**目的**: 値解決後のパスが相対パスの場合、その設定ファイルが置かれている場所を基準に解決する。

**実装方針**:
1.  **コンテキストの保持**:
    -   設定ファイルをマージする際、および `ExpressionResolver` が値を解決する際に、その値がどの設定ファイルから来たものか（ファイルの絶対パス）を追跡できるようにする。
2.  **パスの変換**:
    -   `ExpressionResolver` が式の解決を終えた後、対象のキー（`volumes`のホストパス、`logging.file`など）の値が `./` や `../` で始まるかチェックする。
    -   相対パスの場合、その値の出所となった設定ファイルのディレクトリパスを基準に、`filepath.Join` を使って絶対パスに変換する。
    -   `filepath.Clean` を使って、`../` などを含むパスを正規化する。
