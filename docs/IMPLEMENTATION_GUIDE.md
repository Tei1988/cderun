# cderun 実装ガイド

## 概要

このドキュメントは`cderun`の実装を段階的に進めるためのガイドです。
各ステップは依存関係を考慮して順序付けられており、前のステップが完了してから次に進みます。

## 現在の実装状況

### 実装済み

- 基本的なCLI構造（Cobra使用）
- 厳密な引数解析（P1/P2分離、Hoisting処理）
- ポリグロットエントリーポイント（シンボリックリンク検出）
- 基本的なコンテナ実行（Docker API使用）
- コンテナのライフサイクル管理（作成・起動・待機・削除）
- 標準入出力のアタッチ（TTY/インタラクティブ対応）
- 設定ファイル読み込み（`.cderun.yaml`, `.tools.yaml`）
- イメージマッピング（サブコマンドからイメージ名の解決）
- 優先順位解決ロジック（CLI, Env, YAML, Defaults の6階層）
- ドライランモード（`--dry-run` による設定のプレビュー、YAML/JSON/Simple形式対応）
- 環境変数処理（直接指定およびホストからのパススルー）
- ソケット・バイナリマウント、ツールマウント（自動バイナリマウント含む）
- コンテナ内作業ディレクトリ指定（`--workdir`）
- シグナルハンドリング（SIGINT/SIGTERMの転送、二段階終了）
- TTYリサイズ同期（SIGWINCH対応）
- 詳細ログ機能（レベル別出力、JSON形式対応、デフォルトWARN）
- Podmanフルサポート（Docker互換API経由）
- Docker互換フラグの拡充（ポート、リソース制限、セキュリティ、プルポリシー等）
- ネスト実行サポート（スナップショット注入、逆パス解決）
- 信頼性向上テスト（TTYリサイズ、プルリトライ、ネストE2E）

### 未実装

- なし

## 実装フェーズ

### Phase 1: コア機能（必須） (完了)

基本的なコンテナ実行機能を実装

### Phase 2: 設定管理 (完了)

設定ファイルとイメージマッピング

### Phase 3: 高度な機能 (完了)

環境変数、マウント機能など

### Phase 4: 利便性向上 (完了)

ドライラン、インタラクティブ性の向上、ログ、Podmanサポート

### Phase 5: Docker互換フラグの拡充 (完了)

ポートマッピング、リソース制限、セキュリティ、プルポリシー等の詳細設定

### Phase 6: 高度な設定解決機能 (完了)

このフェーズでは、設定ファイルの検索能力を向上させ、より動的で柔軟な値の解決メカニズムを導入する。

### Step 6.1: 設定ファイルの検索とマージ (完了)

**目的**: 親ディレクトリへの探索機能を追加し、見つかった複数の設定ファイルを階層的にマージする。

### Step 6.2: 値の動的解決 (cderun Expressions) (完了)

**目的**: 設定ファイル内の値を動的に解決する「cderun Expressions」 (`{{...}}`) を実装する。

### Step 6.3: パスの解決 (チルダ・相対パス) (完了)

**目的**: 値解決後のパス文字列に対し、チルダ展開と相対パス解決を行う。

### Phase 7: 機能改善と拡張 (完了)

利便性向上、厳密モードの導入

### Step 7.1: TTYフラグの短縮形 `-t` の追加 (完了)

### Step 7.2: 環境変数の厳密モード `strictEnv` の実装 (完了)

### Phase 8: インテグレーションテスト (完了)

**目的**: インプロセス実行ヘルパーを利用した、実際のコンテナランタイム（Docker/Podman）連携の検証。

### Phase 9: テストカバレッジ計測 (完了)

**目的**: テストの網羅率を可視化し、品質を維持する。

---

### Phase 10: 引数解釈とコマンド組み立ての改善 (完了)

このフェーズでは、コマンドライン引数の解釈ルールを改善し、コンテナに渡されるコマンドの組み立て方をより明確にした。

### Step 10.1: サブコマンドのキー化とイメージ解決 (完了)

**目的**: サブコマンドを純粋なキーとして扱い、イメージ名を解決する。

### Step 10.2: コンテナコマンドの組み立て (完了)

**目的**: `subcommand` をコンテナコマンドに含めず、その後の引数のみを渡すようにする。

---

### Phase 11: ネスト実行 (完了)

このフェーズでは、コンテナ内での `cderun` の再帰的な実行をサポートした。

### Step 11.1: ホストコンテキストの定義 (完了)

**目的**: ネストされた `cderun` がホストの状態を把握するための構造体を定義する。

### Step 11.2: スナップショット管理 (完了)

**目的**: 設定ファイルとホストコンテキストをコンテナに渡すためのスナップショットを作成する。

### Step 11.3: 逆パス解決 (完了)

**目的**: コンテナ内のパスを基底ホストのパスに翻訳し、ネストされたコンテナでのマウントを可能にする。

---

### Phase 12: 信頼性と品質の向上 (完了)

このフェーズでは、コードの堅牢性を高めるためのテスト拡充と、設計仕様に合わせたクリーンアップを行った。

### Step 12.1: ロギング仕様の整合 (完了)

**目的**: ファイル出力等の非推奨機能を削除し、ネスト実行時の安定性を向上させる。

### Step 12.2: TTYリサイズ (SIGWINCH) の検証 (完了)

**目的**: インタラクティブ実行時のウィンドウサイズ変更同期をテストで保証する。

### Step 12.3: ランタイムエラーとリトライの検証 (完了)

**目的**: イメージプル時の `toomanyrequests` 等の異常系処理が正しく動作することを保証する。

### Step 12.4: ネスト実行のシナリオテスト (完了)

**目的**: 複雑なパス解決を含むネスト実行フロー全体を検証する。

## 実装チェックリスト

### Phase 1 (完了)

- [x] Step 1.1: ContainerConfig定義
- [x] Step 1.2: Runtimeインターフェース
- [x] Step 1.3: Docker API実装
- [x] Step 1.4: 基本実行フロー

### Phase 2 (完了)

- [x] Step 2.1: 設定ファイル読み込み
- [x] Step 2.2: イメージマッピング
- [x] Step 2.3: 優先順位解決

### Phase 3 (完了)

- [x] Step 3.1: 環境変数パススルー
- [x] Step 3.2: ソケットマウント
- [x] Step 3.3: cderunバイナリマウント
- [x] Step 3.4: ツールマウント

### Phase 4 (完了)

- [x] Step 4.1: ドライランモード
- [x] Step 4.2: インタラクティブ性の向上（シグナル、リサイズ）
- [x] Step 4.3: ログ・デバッグ
- [x] Step 4.4: Podmanサポート

### Phase 5 (完了)

- [x] Step 5.1: Docker互換フラグの追加（ネットワーク、リソース、セキュリティ等）

### Phase 6 (完了)

- [x] Step 6.1: 設定ファイルの検索とマージ
- [x] Step 6.2: 値の動的解決 (cderun Expressions)
- [x] Step 6.3: パスの解決 (チルダ・相対パス)

### Phase 7 (完了)

- [x] Step 7.1: TTYフラグの短縮形 `-t` の追加
- [x] Step 7.2: 環境変数の厳密モード `strictEnv` の実装

### Phase 8 (完了)

- [x] Step 8.1: テストヘルパー（`runCderun`, `setupTestDir`）の導入
- [x] Step 8.2: 各種インテグレーションテストの実装

### Phase 9 (完了)

- [x] Step 9.1: Makefile の作成とカバレッジ計測の自動化

### Phase 10 (完了)

- [x] Step 10.1: サブコマンドのキー化とイメージ解決
- [x] Step 10.2: コンテナコマンドの組み立て

### Phase 11 (完了)

- [x] Step 11.1: ホストコンテキストの定義
- [x] Step 11.2: スナップショット管理
- [x] Step 11.3: 逆パス解決

### Phase 12 (完了)

- [x] Step 12.1: ロギング仕様の整合
- [x] Step 12.2: TTYリサイズ (SIGWINCH) の検証
- [x] Step 12.3: ランタイムエラーとリトライの検証
- [x] Step 12.4: ネスト実行のシナリオテスト

## 各ステップの完了基準

1. **コードが動作する**: 実装した機能が期待通りに動作
2. **テストが通る**: ユニットテストとインテグレーションテスト
3. **ドキュメントと一致**: featuresドキュメントの仕様通り
4. **エラーハンドリング**: 適切なエラーメッセージ

## 注意事項

- 各ステップは独立してテスト可能にする
- 前のステップが完了してから次に進む
- 既存のコードを壊さない
- featuresドキュメントを常に参照する
