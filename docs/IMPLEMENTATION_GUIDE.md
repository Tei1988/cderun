...
**目的**: 指定した環境変数が欠落している場合の予期せぬ動作を防止する。
...
---
### Phase 10: 引数解釈とコマンド組み立ての改善 (New)

このフェーズでは、コマンドライン引数の解釈ルールを改善し、コンテナに渡されるコマンドの組み立て方をより明確にする。

### Step 10.1: サブコマンドのキー化とイメージ解決

**目的**: サブコマンドを純粋なキーとして扱い、イメージ名を厳格に解決する。

**実装方針**:
1.  `internal/command/root.go` および `internal/config/resolver.go` を修正。
2.  サブコマンドは、`.tools.yaml` から設定を読み込むためのキーとしてのみ使用する。
3.  ツール定義が見つかった場合: ツール定義の `image` が最初の候補となる。
4.  ツール定義が見つからなかった場合: `image` の初期値は空となる。
5.  コマンドラインで `--image` フラグが指定されている場合、上記で決定された `image` 名を上書きする。
6.  最終的に `image` が特定できなかった場合（ツール定義にもなく、`--image` も指定されていない場合）、エラーとして終了する。

### Step 10.2: コンテナコマンドの組み立て

**目的**: `subcommand` をコンテナコマンドに含めず、その後の引数のみを渡すようにする。

**実装方針**:
1.  `internal/command/root.go` および `internal/container/config.go` を修正。
2.  `subcommand` はコンテナに渡すコマンド（`CMD`）には含めない。
3.  コンテナに渡すコマンド（`CMD`）は、常に `subcommand` 以降の引数のみとする。
4.  `ENTRYPOINT` は、ツール定義または `--entrypoint` / `--cderun-entrypoint` フラグで上書きする。上書きがなければ、イメージのデフォルト値を使う。
