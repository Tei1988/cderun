...
`cderun` では AI エージェント等の並行開発におけるコード競合（コンフリクト）を極力防ぐため、**「コード自動生成（Code Generation）」による「Single Source of Truth（単一定義源）」設計**を導入しています。

新しいコマンドラインオプションを追加する際は、CLI オプション構造体（`CLIOptions`）、ルートフラグの Cobra へのバインディング（`rootFlags`）、およびマッピングヘルパー関数のボイラープレートを手書きする必要はありません。定義を1箇所追加してコード生成を行うだけで、これらが自動構築されます。ただし、P3〜P6の設定解決、`ToolConfig`・`ConfigDefaults`・`ResolvedConfig` へのフィールドマッピング、および `DeepCopy` などの設定配線は手動で行う必要があります。
