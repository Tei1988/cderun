...
### 4.1. 厳密な検証 (Strict Validation)

テストの期待値と実際の値を比較する際は、曖昧な検証を避け、可能な限り厳密な比較を行ってください。

- **弱い検証の回避**: `assert.NotEmpty` などの「値が存在すること」だけの検証は避け、期待される具体的な値（例: `assert.Equal(t, expectedPath, actualPath)`) と比較してください。
- **スライスの完全一致**: スライスの検証では、要素が含まれていることだけでなく、長さや全要素が期待通りであることを `assert.Equal` 等で検証してください。
- **正確なパス比較**: ファイルパスの検証では、正規化された絶対パス同士を比較するなど、環境に依存しない確実な方法をとってください。
...
要素の順序が重要でないスライスの検証には `assert.ElementsMatch` を使用してください。これにより、期待されるすべての要素が過不足なく含まれていることを確認できます。
