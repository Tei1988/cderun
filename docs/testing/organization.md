...
## 2. 現状の分析 (2026年3月11日時点)

### 2.1. パッケージ別カバレッジ

| パッケージ | カバレッジ率 | 備考 |
| :--- | :--- | :--- |
| `internal/command` | 90.1% | コアロジック、フラグ解析、ドライラン、ネスト実行等は良好。 |
| `internal/config` | 88.9% | 設定の読み込み、マージ、Expression解決、パス解決等は良好。 |
...
| **合計** | **88.9%** | 全体として 88% を超える極めて高いカバレッジを維持。 |
