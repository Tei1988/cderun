...
| パッケージ | カバレッジ率 | 備考 |
| :--- | :--- | :--- |
| `internal/command` | 88.8% | コアロジック、フラグ解析、ドライラン、ネスト実行等は良好。 |
| `internal/config` | 88.8% | 設定の読み込み、マージ、Expression解決、パス解決等は良好。 |
...
| **合計** | **88.4%** | 全体として 88% を超える極めて高いカバレッジを維持。 |
