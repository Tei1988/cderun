...
## 2. 現状の分析 (2026年3月16日時点)

### 2.1. パッケージ別カバレッジ

| パッケージ | カバレッジ率 | 備考 |
| :--- | :--- | :--- |
| `internal/command` | 90.1% | コアロジック、フラグ解析、ドライラン、ネスト実行、異常系を網羅。 |
| `internal/config` | 90.9% | 設定の読み込み、マージ、Expression解決、パス解決、エラー系を網羅。 |
| `internal/logging` | 97.1% | 極めて高いカバレッジを維持。 |
| `internal/runtime` | 86.5% | リトライロジック、TTYリサイズ、ストリーム処理、APIエラー処理を網羅。 |
| `internal/container` | 0% (ステートメントなし) | 実行ステートメントを持たない構造体定義のみ。 |
| **合計** | **89.5%** | 全体として 89% を超える極めて高いカバレッジを達成。 |
